...
compiler.compileNoGlobals = function (content, filePath, compilers, options, cb) {
  ...
  Promise.all([
    processTemplate(parts.template, filePath, resolvedParts, compilers),
    processScript(parts.script, filePath, resolvedParts, compilers)
  ].concat(parts.styles.map(function (style) {
    return processStyle(style, filePath, id, resolvedParts, compilers, options)
  })))
  .then(mergeParts)
  .catch(cb)
  ...
  function mergeParts () {
    ...
    if (style && !isServer) {
      ...
      if (!options.extractCSS) {
        ...
        output +=
          'var __vueify_style_dispose__ = require("@paulcbetts/' + insertCSSPath + '").insert(' + style + ')\n'
      }
    }
    // script
...
function processTemplate (part, filePath, parts, compilers) {
  if (!part) return Promise.resolve()
  var template = getContent(part, filePath)

  return compileAsPromise('template', template, part.lang, filePath, compilers)
    .then(function (res) {
      parts.template = compileTemplate(res, compiler)
    })
}

function processScript (part, filePath, parts, compilers) {
  if (!part) return Promise.resolve()
  var lang = part.lang || 'js';
  var script = getContent(part, filePath)

  return compileAsPromise('script', script, lang, filePath, compilers)
    .then(function (res) {
...
function processStyle (part, filePath, id, parts, compilers, options) {
  var style = getContent(part, filePath)
  return compileAsPromise('style', style, part.lang, filePath, compilers)
    .then(function (res) {
      res = res.trim()
...
function compileAsPromise (type, source, lang, filePath, compilers) {
